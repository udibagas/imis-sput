    ...
    const app = new Vue({
        ...
        methods: {
            ...
            store: function() {
                ...
                axios.post('{{url("buyer")}}', this.formData).then(function(r) {
                    unblock('form');
            ...
            edit: function(id) {
                var t = this;
                this.formTitle = "EDIT BUYER";
                this.formErrors = {};
                this.error = {};

                axios.get('{{url("buyer")}}/' + id).then(function(r) {
                    t.formData = r.data;
                    $('#modal-form').modal('show');
                })
            ...
            update: function() {
                block('form');
                var t = this;
                axios.put('{{url("buyer")}}/' + this.formData.id, this.formData).then(function(r) {
                    unblock('form');
                    $('#modal-form').modal('hide');
            ...
            var grid = $('#bootgrid').bootgrid({
                ...
                formatters: {
                    "commands": function(column, row) {
                        return '@can("update", App\Buyer::class) <a href="#" class="btn btn-info btn-xs c-edit" data-id="'+row.id+'"><i class="icon-pencil"></i></a> @endcan' +
                            '@can("delete", App\Buyer::class) <a href="#" class="btn btn-danger btn-xs c-delete" data-id="'+row.id+'"><i class="icon-trash"></i></a> @endcan';
                    }
                }
