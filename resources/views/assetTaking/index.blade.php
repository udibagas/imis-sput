        ...
        <table class="table table-striped table-hover " id="bootgrid" style="border-top:2px solid #ddd">
            <thead>
                <tr>
                    ...
                    <th data-column-id="old_location">Old Location</th>
                    <th data-column-id="new_location">New Location</th>
                    <th data-column-id="old_status">Old Status</th>
                    <th data-column-id="new_status">New Status</th>
...
const app = new Vue({
    ...
    data: {
        ...
        formTitle: '',
        error: {},
        assets: {!! App\Asset::selectRaw('*, CONCAT(reg_no, " - ", name) AS text')
            ->orderBy('reg_no', 'ASC')->get() !!},
    ...
    watch: {
        'formData.asset_id': function(v, o) {
            if (!this.formData.id) {
                var asset = this.assets.filter(function(a) {
                    return a.id == v;
                })[0];

                this.formData.old_asset_location_id = this.formData.new_asset_location_id = asset.asset_location_id;
                this.formData.old_asset_status_id = this.formData.new_asset_status_id = asset.asset_status_id;
            }
        }
    },
    methods: {
